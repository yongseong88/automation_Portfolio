...
class TestCart():# BaseTest 상속 없이도 됨
    ...
    def test_cart_order(self):
        ...
        cart_page.go_to_order()  # '주문하기' → 주문서(/order)로 이동

        # 2순위: 주문서 진입 + 주문 후 장바구니 비워짐
        assert cart_page.check_url(f"{self.base_url}/order"), "주문하기 후 주문서로 이동하지 않음"
    ...
    def test_cart_shipping_fee_under_threshold(self):
        ...
        shipping_value = product_action.shipping_verification()
        cart_page.go_to_cart()
        shipping_txt = cart_page.shipping(shipping_value)
        ...
        assert cart_response.status == 200, "장바구니 조회 api 호출 실패"
        assert shipping_value == 3000, f"4만원 미만인데 배송비가 3,000원이 아님: api={shipping_value}"

        # 2순위: 화면 배송비 텍스트가 API 값과 일치
        assert shipping_txt, f"화면 배송비와 API 배송비 불일치 api={shipping_value}"
    ...
    def test_cart_shipping_free_over_threshold(self):
        ...
        shipping_value = product_action.shipping_free()
        cart_page.go_to_cart()
        shipping_txt = cart_page.shipping(shipping_value)
        ...
        # 1순위: API status + 배송비 값
        assert cart_response.status == 200, "장바구니 조회 api 호출 실패"
        assert shipping_value == 0, f"4만원 이상인데 배송비가 무료가 아님: api={shipping_value}"
